...
def _unique_run(results_root: Path, prefix: str) -> Path:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"expected exactly one run matching {prefix!r}; "
...
def analyze_mode(
    ...
    *,
    ...
    if cfg.trace_format != spec.expected_trace_format:
        raise ValueError(
            f"{spec.label}: expected trace_format={spec.expected_trace_format}, "
            f"got {cfg.trace_format}"
        )
    ...
    for endpoint in discovery:
        ...
        print(
            ...
            f"confirmation Logistic="
            f"{chosen['confirmation_logistic_balanced_accuracy']:.4f}, "
...
def main() -> None:
    ...
    chosen_parts = []
    for mode, mode_layers in selected.items():
        for endpoint, layer in mode_layers.items():
            chosen_parts.append(
                metrics[
                    metrics["comparison_mode"].eq(mode)
                    & metrics["endpoint"].eq(endpoint)
                    & metrics["layer"].astype(int).eq(int(layer))
                ]
            )
    chosen = pd.concat(chosen_parts, ignore_index=True)
    chosen.to_csv(args.output / "selected_confirmation_summary.csv", index=False)
