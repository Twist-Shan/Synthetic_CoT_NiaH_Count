...
class Vocab:
    ...
    @classmethod
    def build(cls) -> "Vocab":
        if len(ALL_TOKENS) != 90 or len(set(ALL_TOKENS)) != 90:
            raise ValueError("v4 v2-style vocabulary must contain exactly 90 unique tokens.")
    ...
    def count_id(self, value: int) -> int:
        return self.token_to_id[number_token(value)]
...
def trace_index_token(value: int) -> str:
...
def number_token(value: int) -> str:
    if not 1 <= int(value) <= 10:
        raise ValueError(f"number token must be in 1..10, got {value}.")
    return f"<{int(value)}>"
