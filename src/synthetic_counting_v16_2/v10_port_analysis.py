"""v10-style representation and causal analyses for the audited v16.2 RoPE run.
...
The v10 task used abstract marker tokens and counts 1..30.  v16.2 instead uses
...
from .training import load_v16_2_checkpoint_model
...
def load_context(run_dir: str | Path, device: str | None = None) -> RunContext:
    ...
    if config.position_encodings != ("rope",):
        raise ValueError("the v10 port is defined for the audited RoPE-only v16.2 run")
    ...
    for mode in ("nonthinking", "thinking"):
        _, loaded_vocab, loaded_pool, loaded_split, model = load_v16_2_checkpoint_model(
            run_dir, "rope", mode, step=config.train_steps, device=selected_device
        )
        if loaded_vocab.fingerprint != vocab.fingerprint:
            raise ValueError("loaded vocabulary differs between v16.2 model variants")
        if loaded_pool.pool_fingerprint != pool.pool_fingerprint:
            raise ValueError("loaded needle pool differs between v16.2 model variants")
        if loaded_split.split_fingerprint != split.split_fingerprint:
            raise ValueError("loaded corpus split differs between v16.2 model variants")
...
def _balanced(examples: Sequence[V16_2Example], per_count: int) -> list[V16_2Example]:
    buckets: dict[int, list[V16_2Example]] = {count: [] for count in range(1, 11)}
...
def _replace_tokens(item: V16_2Rendered, replacements: dict[int, str], vocab: V16_2Vocab) -> V16_2Rendered:
...
def _truncate(item: V16_2Rendered, length: int) -> V16_2Rendered:
    return replace(
        item,
...
def _non_target_token(example: V16_2Example, vocab: V16_2Vocab) -> str:
...
def _forward(
    ...
    items: Sequence[V16_2Rendered],
    vocab: V16_2Vocab,
    device: str,
    **kwargs: Any,
):
    ids, _, mask = collate_v16_2(list(items), vocab, device)
    with torch.inference_mode():
        return model(input_ids=ids, attention_mask=mask, **kwargs)


def _count_metrics(logits: torch.Tensor, items: Sequence[V16_2Rendered], vocab: V16_2Vocab) -> pd.DataFrame:
...
def _site_positions(items: Sequence[V16_2Rendered], site: str) -> list[list[int]]:
...
def run_head_ablation(ctx: RunContext, options: PortOptions, rankings: dict[str, list[Head]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for role, (mode, site, outcome) in mechanisms.items():
        items = [render_v16_2(example, ctx.vocab, mode) for example in examples]
...
def _retrieval_corruption(example: V16_2Example, vocab: V16_2Vocab, k: int) -> tuple[V16_2Rendered, V16_2Rendered, int, int]:
    clean = render_v16_2(example, vocab, "thinking")
...
def _marker_margin(logits: torch.Tensor, items: Sequence[V16_2Rendered], ks: Sequence[int], target_ids: Sequence[int], alternative_ids: Sequence[int]) -> np.ndarray:
    values = []
...
def _successor_pair(example: V16_2Example, vocab: V16_2Vocab, k: int) -> tuple[V16_2Rendered, V16_2Rendered]:
    clean_full = render_v16_2(example, vocab, "thinking")
...
def run_successor_patching(
    ...
    def prepare(examples: Sequence[V16_2Example]):
    ...
    def captures(examples: Sequence[V16_2Example]):
...
def run_geometry(ctx: RunContext, options: PortOptions) -> tuple[pd.DataFrame, pd.DataFrame, dict[tuple[str, int, int], np.ndarray]]:
    ...
    for mode in ("nonthinking", "thinking"):
        items = [render_v16_2(example, ctx.vocab, mode) for example in train_examples]
...
def _pair_examples(examples: Sequence[V16_2Example], per_count: int = 3) -> list[tuple[V16_2Example, V16_2Example]]:
    buckets: dict[int, list[V16_2Example]] = {count: [] for count in range(1, 11)}
    for example in examples:
        count = int(example.count or 0)
        if count in buckets and len(buckets[count]) < per_count:
            buckets[count].append(example)
    pairs: list[tuple[V16_2Example, V16_2Example]] = []
...
def run_residual_transport(
    ...
    rows: list[dict[str, Any]] = []
    for mode in ("nonthinking", "thinking"):
        receiver_items = [render_v16_2(receiver, ctx.vocab, mode) for receiver, _ in pairs]
        donor_items = [render_v16_2(donor, ctx.vocab, mode) for _, donor in pairs]
...
def run_trace_state_patching(ctx: RunContext) -> pd.DataFrame:
    ...
    for example in ctx.heldout_examples:
        ...
        if count in buckets and len(buckets[count]) < 3:
            buckets[count].append(example)
    pairs: list[tuple[V16_2Example, V16_2Example, int]] = []
    for k in range(2, 9):
    ...
    donor_items = [render_v16_2(donor, ctx.vocab, "thinking") for donor, _, _ in pairs]
    receiver_items = [render_v16_2(receiver, ctx.vocab, "thinking") for _, receiver, _ in pairs]
...
def _conflicts(example: V16_2Example, vocab: V16_2Vocab) -> tuple[V16_2Rendered, dict[str, V16_2Rendered]]:
    clean = render_v16_2(example, vocab, "thinking")
    ...
    return clean, {
        ...
        "shortened_trace_position_shifted": render_v16_2_shortened_trace(example, vocab),
    }
...
def run_state_to_head_routing(ctx: RunContext, targeted_heads: Sequence[Head]) -> pd.DataFrame:
    ...
    items = [render_v16_2(example, ctx.vocab, "thinking") for example in examples]
    output = _forward(
...
def _successor_dataset(
    ctx: RunContext,
    examples: Sequence[V16_2Example],
) -> tuple[list[int], list[V16_2Rendered], list[V16_2Rendered], list[int], list[int]]:
    ks = [max(2, min(int(example.count or 0) - 1, int(example.count or 0) // 2)) for example in examples]
    pairs = [_successor_pair(example, ctx.vocab, k) for example, k in zip(examples, ks, strict=True)]
    clean = [pair[0] for pair in pairs]
    short = [pair[1] for pair in pairs]
