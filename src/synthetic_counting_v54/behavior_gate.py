...
def evaluate_behavior_gate(run_dir: str | Path) -> dict[str, object]:
    return _evaluate_behavior_gate(
        run_dir,
        expected_version="v54",
        expected_final_step=10_000,
    )


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Evaluate the fixed v54 behavior gate")
    parser.add_argument("run_dir", type=Path)
    parser.add_argument("--output", type=Path, default=None)
    args = parser.parse_args(argv)
    result = evaluate_behavior_gate(args.run_dir)
    output = args.output or args.run_dir / "analysis" / "behavior_gate_v54.json"
