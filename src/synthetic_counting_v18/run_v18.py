...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the focused v18 skewed-count explicit-trace suite")
    ...
    parser.add_argument("--out-root", default="runs/synthetic_counting_v18")
...
def main(argv: list[str] | None = None) -> None:
    ...
    run_name = args.run_name or f"v18_{args.preset}_{args.suite}_seed{cfg.seed}"
    ...
    if args.stage in {"all", "train"}:
        ...
        for index, spec in enumerate(specs, 1):
            print(f"[v18] run {index}/{len(specs)}: {spec.name}", flush=True)
