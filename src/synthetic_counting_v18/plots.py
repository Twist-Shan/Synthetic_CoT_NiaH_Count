...
def plot_learning(run_dir: Path, figures: Path) -> None:
    ...
    if not training.empty:
        ...
        axis.set(
            ...
            title="v18 training loss: direct C_n versus explicit I_k, M_k, END, C_n trace",
        )
...
def plot_final(run_dir: Path, figures: Path) -> None:
    ...
    axes[0].set(
        ...
        title="Final scalar count token C_n",
        ylim=(-0.02, 1.02),
    )
    ...
    axes[1].set(
        xlabel="gold needle count",
        title="Exact CoT trace: all I_k and marker identities",
        ylim=(-0.02, 1.02),
    )
    for axis in axes:
        handles, labels = axis.get_legend_handles_labels()
...
def plot_attention(run_dir: Path, figures: Path) -> None:
    ...
    for distribution in sorted(frame["distribution"].unique()):
        ...
        _heatmap(axes[1], targeted, "correct_prompt_needle_mass", "CoT I_k: raw matching-needle mass")
        _heatmap(axes[2], successor, "next_prompt_needle_mass", "CoT M_k: next prompt-needle mass")
        _heatmap(axes[3], readout, "trace_markers_mass", "CoT END: mass on all trace markers")
        figure.suptitle(f"v18 descriptive attention signatures: {distribution}", fontsize=16, fontweight="bold")
