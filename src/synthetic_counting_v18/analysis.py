...
from .data import ANSWER, END, START, ReferenceExample, render, sample_example
...
def _trace_positions(context_length: int, count: int) -> tuple[list[int], list[int], int]:
    index_positions = [context_length + 1 + 2 * offset for offset in range(count)]
    marker_positions = [position + 1 for position in index_positions]
    end_position = context_length + 1 + 2 * count
    return index_positions, marker_positions, end_position
...
def _attention_categories(
    ...
    visible_indices = [position for position in index_positions if position <= query_position]
...
def collect_attention_for_spec(
    ...
    for example_id, example in enumerate(examples):
        ...
        if spec.mode == "direct":
            queries = [("final_answer", spec.context_length, None)]
        else:
            index_positions, marker_positions, end_position = _trace_positions(
                spec.context_length,
                example.count,
            )
            queries = [("final_answer", end_position, None)]
            queries.extend(
...
def _site_positions(
    ...
    if site == "final_answer":
        if spec.mode == "direct":
            return [(spec.context_length, example.count)]
        _, _, end_position = _trace_positions(spec.context_length, example.count)
        return [(end_position, example.count)]
    if spec.mode != "cot":
        return []
