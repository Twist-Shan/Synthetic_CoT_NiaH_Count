...
from .analysis import run_v16_3_attention_analysis, run_v16_3_state_analysis
from .config import V16_3Config, config_from_dict, prepare_run_dir
from .data import (
    CorpusSplit,
    V16_3Example,
    V16_3Vocab,
...
from .plots import make_all_v16_3_plots
from .training import sync_tree, train_v16_3_models
...
def _update_manifest(run_dir: Path, cfg: V16_3Config, stage: str, status: str) -> None:
...
def prepare_v16_3_data(
    ...
    if any(path.exists() for path in (split_path, pool_path, suites_path)):
        ...
        return load_prepared_v16_3_data(cfg, vocab, text, run_dir)
...
def load_prepared_v16_3_data(
    ...
    if missing:
        raise FileNotFoundError(
            "v16_3 preparation is required before training/analysis; missing " + ", ".join(missing)
        )
...
def run_v16_3_pipeline(
    cfg: V16_3Config,
    *,
    stage: str | Iterable[str] = "all",
    out_root: str | Path = "runs/synthetic_counting_v16_3",
    ...
    if config_path.exists():
        ...
        if replace(saved_cfg, device=cfg.device) != cfg:
            raise ValueError("run directory contains a different v16_3 config; use a new run name")
    else:
    ...
    if vocab_path.exists():
        if V16_3Vocab.load(vocab_path).fingerprint != vocab.fingerprint:
    ...
    for current in _stage_list(stage):
        print(f"[v16_3] stage={current}", flush=True)
        ...
        try:
            with timed_event(
                ...
                if current == "prepare":
                    prepared = prepare_v16_3_data(cfg, vocab, text, run_dir)
                else:
                    if prepared is None:
                        prepared = load_prepared_v16_3_data(cfg, vocab, text, run_dir)
                    ...
                    if current == "train":
                        ...
                        run_v16_3_attention_analysis(cfg, vocab, run_dir, curve_suites["heldout"]["task"])
                    elif current == "state":
                        run_v16_3_state_analysis(
                            cfg,
                            vocab,
                            run_dir,
                        ...
                        make_all_v16_3_plots(cfg, run_dir)
        except Exception:
