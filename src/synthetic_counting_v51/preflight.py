...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit v51 support, permutation, endpoint, and grammar weight."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v51")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v51 requires permuted counting-task contexts")
    if cfg.train_steps != 10_000:
        raise ValueError("v51 requires the fixed 10,000-step endpoint")
    if (cfg.n_layer, cfg.n_head, cfg.n_embd, cfg.n_inner) != (4, 4, 256, 1024):
        raise ValueError("v51 requires the fixed 4L/4H/256D/1024-MLP architecture")
    if not cfg.task_output_trace_delimiters_as_structure:
        raise ValueError("v51 requires grammar-balanced separator partitioning")
    if (
        cfg.task_output_count_weight,
        cfg.task_output_trace_weight,
        cfg.task_output_structure_weight,
    ) != (8.0, 8.0, 16.0):
        raise ValueError("v51 requires count/trace/structure weights 8/8/16")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v51 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v51 audit window was not actually permuted")
    ...
    result.update(
        {
            ...
            "training_change_from_v49": "grammar/structure aggregate weight 8 -> 16 only",
    ...
    output = root / "analysis" / "preflight_v51.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v51 before optimization")
