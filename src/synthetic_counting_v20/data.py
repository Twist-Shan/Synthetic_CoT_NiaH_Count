...
class CorpusSplit:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": "v20_v21_corpus_split_v1",
...
def build_corpus_split(cfg: V20Config, text: str) -> CorpusSplit:
    guard = cfg.seq_len - 1
...
def load_corpus_split(path: str | Path, cfg: V20Config, text: str) -> CorpusSplit:
...
class V20Vocab:
    ...
    @classmethod
    def build(cls, cfg: V20Config, corpus_text: str) -> "V20Vocab":
        ...
        numbers = (
            [f"<{index}>" for index in range(1, cfg.count_max_threshold + 1)]
            if cfg.count_tokenization == "atomic"
            else [f"<D{index}>" for index in range(10)]
        )
        tokens = special + characters + numbers
        if len(tokens) != len(set(tokens)):
            raise ValueError("v20 vocabulary contains duplicate tokens")
        return cls(
    ...
    def number_token(self, value: int) -> str:
        if self.count_tokenization != "atomic":
            raise ValueError("number_token is defined only for v20 atomic numbers; use number_tokens")
    ...
    def number_tokens(self, value: int) -> tuple[str, ...]:
        value = int(value)
        if value < 1:
            raise ValueError("number must be positive")
        if self.count_tokenization == "atomic":
            return (self.number_token(value),)
        return tuple(f"<D{digit}>" for digit in str(value))

    def decode_number_tokens(self, tokens: Iterable[str]) -> int | None:
        values = list(tokens)
        if self.count_tokenization == "atomic":
            if len(values) != 1 or values[0] not in self.numbers:
                return None
            return self.numbers.index(values[0]) + 1
        if not values or any(token not in self.numbers for token in values):
            return None
        digits = "".join(token[2:-1] for token in values)
    ...
    @classmethod
    def load(cls, path: str | Path) -> "V20Vocab":
...
class V20Spans:
    ...
    trace_index_token_groups: tuple[tuple[int, ...], ...]
    ...
    def count_pos(self) -> int:
        """First answer-number token; retained for atomic-v20 causal compatibility."""
...
def _trace_tokens_and_positions(
    ...
    vocab: V20Vocab,
    trace_start: int,
) -> tuple[list[str], tuple[tuple[int, ...], ...], tuple[int, ...], tuple[int, ...]]:
    tokens: list[str] = []
    ...
    for index, marker in enumerate(markers, start=1):
        ...
        cursor += 1
    return tokens, tuple(groups), tuple(anchors), tuple(marker_positions)
...
def render_v20(example: V20Example, vocab: V20Vocab, mode: str) -> V20Rendered:
    ...
    if mode == "nonthinking":
        answer_number = list(vocab.number_tokens(example.count))
        tokens = ["<BOS>", *task_prefix, *example.seq_tokens, "<Ans>", *answer_number, "<EOS>"]
        ans_pos = prompt_end
        count_positions = tuple(range(ans_pos + 1, ans_pos + 1 + len(answer_number)))
        eos_pos = count_positions[-1] + 1
        spans = V20Spans(
            0, prompt_start, prompt_end, None, (), (), (), None, ans_pos, count_positions, eos_pos,
            tuple(range(1, prompt_start)),
        )
    else:
        think_pos = prompt_end
        trace_start = think_pos + 1
        trace, index_groups, index_positions, marker_positions = _trace_tokens_and_positions(
            example.needle_markers, vocab, trace_start
        )
        ...
        tokens = [
            ...
            "</Think>", "<Ans>", *answer_number, "<EOS>",
        ]
        spans = V20Spans(
            0, prompt_start, prompt_end, think_pos, index_positions, index_groups,
            marker_positions, close_pos, ans_pos, count_positions, eos_pos,
            tuple(range(1, prompt_start)),
        )
    ids = vocab.encode(tokens)
...
def render_v20_shortened_trace(
    example: V20Example, vocab: V20Vocab
) -> V20Rendered:
    ...
    gold = render_v20(example, vocab, "thinking")
    ...
    trace_start = think_pos + 1
    trace, index_groups, index_positions, marker_positions = _trace_tokens_and_positions(
        shortened_markers, vocab, trace_start
    )
    close_pos = trace_start + len(trace)
    ans_pos = close_pos + 1
    answer_number = list(vocab.number_tokens(example.count))
    ...
    for row, item in enumerate(rendered):
        ...
        for position in item.spans.count_positions:
            if weights[row, position] > 0:
                weights[row, position] = float(cfg.final_count_loss_weight)
        for position in (
...
def component_target_positions(item: V20Rendered) -> dict[str, tuple[int, ...]]:
    ...
    components: dict[str, tuple[int, ...]] = {
        ...
        "final_count": spans.count_positions,
        "eos": (spans.eos_pos,),
...
def example_to_dict(example: V20Example) -> dict[str, Any]:
    value = asdict(example)
    return value


def example_from_dict(value: dict[str, Any]) -> V20Example:
    data = dict(value)
    for name in ("needle_characters", "rendered_set_order", "needle_positions", "needle_markers", "per_character_counts"):
        if data.get(name) is not None:
            data[name] = tuple(data[name])
    return V20Example(**data)


def _balanced_prefix(examples: list[V20Example], count: int, seed: int) -> list[V20Example]:
    by_count: dict[int, list[V20Example]] = {}
...
def build_test_suite_manifests(
    ...
    split: CorpusSplit,
    pool: NeedlePool,
) -> dict[str, list[V20Example]]:
    suite_size = cfg.final_examples_per_count * cfg.count_max_threshold
    raw = raw_v20_examples(cfg, vocab, text, split, suite_size, cfg.seed + 80_000, region_name="test")
    task = balanced_v20_examples(
        cfg, vocab, text, split, pool, cfg.final_examples_per_count, cfg.seed + 81_000,
        region_name="test",
    )
    task_count = round(cfg.task_occurrence_ratio * suite_size)
...
def save_suite_manifests(
    ...
    *,
    ...
    obj = {
        "schema_version": "v20_v21_suite_manifest_v1",
...
) -> tuple[dict[str, dict[str, list[V20Example]]], dict[str, list[V20Example]]]:
    obj = json.loads(Path(path).read_text(encoding="utf-8"))
