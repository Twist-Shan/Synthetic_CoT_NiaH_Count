...
def _attention_categories(item: V20Rendered, weights: np.ndarray) -> dict[str, float]:
...
def run_v20_attention_analysis(
    cfg: V20Config,
    vocab: V20Vocab,
    run_dir: Path,
    heldout_task_examples: list[V20Example],
) -> None:
    examples: list[V20Example] = []
    ...
    for position_encoding, mode in cfg.model_variants:
        ...
        _, loaded_vocab, _, model = load_final_v20_model(
...
def _sites(item: V20Rendered, mode: str) -> list[tuple[str, int, int]]:
...
def collect_v20_states(
    ...
    *,
    ...
    planned: list[tuple[V20Rendered, list[tuple[str, int, int]]]] = []
    ...
    for start in range(0, len(planned), batch_size):
        ...
        ids, _, attention_mask = collate_v20(
...
_collect_states = collect_v20_states
_nearest_centroid = nearest_centroid
_ridge_prediction = ridge_prediction


def run_v20_state_analysis(
    cfg: V20Config,
    vocab: V20Vocab,
    run_dir: Path,
    train_task_examples: list[V20Example],
    heldout_task_examples: list[V20Example],
) -> None:
    rows: list[dict[str, Any]] = []
    for position_encoding, mode in cfg.model_variants:
