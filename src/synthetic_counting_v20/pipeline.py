...
from .analysis import run_v20_attention_analysis, run_v20_state_analysis
from .config import V20Config, config_from_dict, prepare_run_dir
from .data import (
    CorpusSplit,
    V20Example,
    V20Vocab,
...
from .plots import make_all_v20_plots
...
def _update_manifest(run_dir: Path, cfg: V20Config, stage: str, status: str) -> None:
...
def prepare_v20_data(
    ...
    if any(path.exists() for path in (split_path, pool_path, suites_path)):
        ...
        return load_prepared_v20_data(cfg, vocab, text, run_dir)
...
def load_prepared_v20_data(
    ...
    if missing:
        raise FileNotFoundError(
            f"{cfg.version} preparation is required before training/analysis; missing "
            + ", ".join(missing)
        )
...
def run_v20_pipeline(
    cfg: V20Config,
    *,
    stage: str | Iterable[str] = "all",
    out_root: str | Path = "runs/synthetic_counting_v20",
    ...
    if config_path.exists():
        ...
        if replace(saved_cfg, device=cfg.device) != cfg:
            raise ValueError(
                f"run directory contains a different {cfg.version} config; use a new run name"
            )
    else:
    ...
    if vocab_path.exists():
        if V20Vocab.load(vocab_path).fingerprint != vocab.fingerprint:
    ...
    for current in _stage_list(stage):
        ...
        try:
            with timed_event(
                ...
                if current == "prepare":
                    prepared = prepare_v20_data(cfg, vocab, text, run_dir)
                else:
                    if prepared is None:
                        prepared = load_prepared_v20_data(cfg, vocab, text, run_dir)
                    ...
                    if current == "train":
                        ...
                        run_v20_attention_analysis(cfg, vocab, run_dir, curve_suites["heldout"]["task"])
                    elif current == "state":
                        run_v20_state_analysis(
                            cfg,
                            vocab,
                            run_dir,
