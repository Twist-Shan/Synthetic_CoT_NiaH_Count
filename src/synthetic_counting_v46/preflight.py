...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit full support plus the count-preserving task-window permutation."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v46")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v46 requires permuted counting-task contexts")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v46 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v46 audit window was not actually permuted")
    ...
    output = root / "analysis" / "preflight_v46.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v46 before optimization")
