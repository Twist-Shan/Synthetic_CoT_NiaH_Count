...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit v58 support, permutation, endpoint, and parallel head bank."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v58")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v58 requires permuted counting-task contexts")
    if cfg.train_steps != 10_000:
        raise ValueError("v58 requires the fixed 10,000-step endpoint")
    if cfg.batch_size != 128:
        raise ValueError("v58 requires batch size 128")
    if (cfg.n_layer, cfg.n_head, cfg.n_embd, cfg.n_inner) != (4, 8, 512, 2048):
        raise ValueError("v58 requires the fixed 4L/8H/512D/2048-MLP architecture")
    if cfg.n_embd // cfg.n_head != 64:
        raise ValueError("v58 requires 64 dimensions per attention head")
    if not cfg.task_output_trace_delimiters_as_structure:
        raise ValueError("v58 requires grammar-balanced separator partitioning")
    if (
        cfg.task_output_count_weight,
        cfg.task_output_trace_weight,
        cfg.task_output_structure_weight,
    ) != (8.0, 8.0, 16.0):
        raise ValueError("v58 requires count/trace/structure weights 8/8/16")
    if cfg.task_output_scheduled_sampling_max_probability != 0.0:
        raise ValueError("v58 requires pure gold-prefix teacher forcing")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v58 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v58 audit window was not actually permuted")
    ...
    result.update(
        {
            ...
            "serialized_trace_changed_from_v57": False,
            "training_change_from_v57": (
                "parallel head bank 6H/384D/1536-MLP -> 8H/512D/2048-MLP only"
            ),
    ...
    output = root / "analysis" / "preflight_v58.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v58 before optimization")
