...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit v49 support, permutation, horizon, and loss partition."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v49")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v49 requires permuted counting-task contexts")
    if cfg.train_steps != 10_000:
        raise ValueError("v49 requires the fixed 10,000-step endpoint")
    if (cfg.n_layer, cfg.n_head, cfg.n_embd, cfg.n_inner) != (4, 4, 256, 1024):
        raise ValueError("v49 requires the fixed 4L/4H/256D/1024-MLP architecture")
    if not cfg.task_output_trace_delimiters_as_structure:
        raise ValueError("v49 requires grammar-balanced separator partitioning")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v49 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v49 audit window was not actually permuted")
    ...
    result.update(
        {
            ...
            "nonthinking_objective_changed_from_v47": False,
            "serialized_trace_changed_from_v47": False,
    ...
    output = root / "analysis" / "preflight_v49.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v49 before optimization")
