...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit v50 support, permutation, horizon, and loss partition."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v50")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v50 requires permuted counting-task contexts")
    if cfg.train_steps != 20_000 or cfg.lr_decay_steps is not None or cfg.min_lr != 0:
        raise ValueError("v50 requires the fixed 20,000-step cosine-to-zero horizon")
    if (cfg.n_layer, cfg.n_head, cfg.n_embd, cfg.n_inner) != (4, 4, 256, 1024):
        raise ValueError("v50 requires the fixed 4L/4H/256D/1024-MLP architecture")
    if not cfg.task_output_trace_delimiters_as_structure:
        raise ValueError("v50 requires grammar-balanced separator partitioning")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v50 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v50 audit window was not actually permuted")

    result.update(
        {
            "training_endpoint": 20_000,
            "cosine_decay_endpoint": 20_000,
            "architecture": {
                "n_layer": 4,
                "n_head": 4,
                "n_embd": 256,
            ...
            "task_output_component_partition": {
                ...
                "trace": "retrieved marker identities only",
                "structure": "<Think>, repeated <Sep>, </Think>, <Ans>, and <EOS>",
            },
            "serialized_trace_changed_from_v49": False,
            "training_change_from_v49": "cosine horizon 10000 -> 20000 only",
    ...
    output = root / "analysis" / "preflight_v50.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v50 before optimization")
