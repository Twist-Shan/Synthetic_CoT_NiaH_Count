...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit v56 support, permutation, endpoint, and batch size."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v56")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v56 requires permuted counting-task contexts")
    if cfg.train_steps != 10_000:
        raise ValueError("v56 requires the fixed 10,000-step endpoint")
    if cfg.batch_size != 256:
        raise ValueError("v56 requires batch size 256")
    if (cfg.n_layer, cfg.n_head, cfg.n_embd, cfg.n_inner) != (4, 4, 256, 1024):
        raise ValueError("v56 requires the fixed 4L/4H/256D/1024-MLP architecture")
    if not cfg.task_output_trace_delimiters_as_structure:
        raise ValueError("v56 requires grammar-balanced separator partitioning")
    if (
        cfg.task_output_count_weight,
        cfg.task_output_trace_weight,
        cfg.task_output_structure_weight,
    ) != (8.0, 8.0, 16.0):
        raise ValueError("v56 requires count/trace/structure weights 8/8/16")
    if cfg.task_output_scheduled_sampling_max_probability != 0.0:
        raise ValueError("v56 requires pure gold-prefix teacher forcing")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v56 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v56 audit window was not actually permuted")

    result.update(
        {
            "training_endpoint": 10_000,
            "optimization_batch_size": 256,
            "training_examples_per_mode": 2_560_000,
            "equal_compute_to_v51": False,
            "architecture": {
                "n_layer": 4,
                "n_head": 4,
                "n_embd": 256,
            ...
            "training_change_from_v51": "optimization batch size 128 -> 256 only",
    ...
    output = root / "analysis" / "preflight_v56.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v56 before optimization")
