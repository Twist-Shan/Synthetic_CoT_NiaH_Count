...
def greedy_batch(
    ...
    width = number_width(spec.eval_max_count)
    maximum_new = (
        1 + width + 1
        if spec.mode == "direct"
        else spec.eval_max_count * (2 + width) + 2 + width + 1
    )
    for _ in range(maximum_new):
        ...
        for row, token in enumerate(next_tokens):
            ...
            if token == NUM_END and (
                spec.mode == "direct"
                or (END in suffix and COUNT in suffix[suffix.index(END) + 1 :])
            ):
                active[row] = False
    ...
    for row, tokens in enumerate(generated):
        ...
        enumeration, token_count, generated_markers = parse_generated_suffix(
            suffix,
            spec.mode,
            spec.eval_max_count,
        )
...
def parse_generated_suffix(
    suffix: list[int] | tuple[int, ...],
    mode: str,
    maximum_count: int,
) -> tuple[int | None, int | None, list[int]]:
    """Parse one free-running completion under the shared-digit grammar."""

    tokens = list(suffix)
    if mode == "direct":
        token_count = None
        if len(tokens) >= 3 and tokens[0] == COUNT and tokens[-1] == NUM_END:
            token_count = decode_number(tokens[1:-1], maximum_count)
    ...
    while cursor < len(tokens) and tokens[cursor] != END:
        ...
        cursor += 1
        digit_start = cursor
        while cursor < len(tokens) and digit_from_token(tokens[cursor]) is not None:
            cursor += 1
        index = decode_number(tokens[digit_start:cursor], maximum_count)
    ...
    enumeration = (
        ...
        if valid_trace and cursor < len(tokens) and tokens[cursor] == END
        else None
    )
    token_count = None
    if (
        ...
        and tokens[cursor + 1] == COUNT
        and tokens[-1] == NUM_END
    ):
        token_count = decode_number(tokens[cursor + 2 : -1], maximum_count)
    return enumeration, token_count, generated_markers
