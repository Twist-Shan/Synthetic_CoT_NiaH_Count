...
class ReferenceConfig:
    ...
    def vocab_size(self) -> int:
        # Add INDEX, COUNT, NUM_END, and ten shared decimal digits D0..D9.
        return 283
    ...
    def validate(self) -> None:
        ...
        if (self.noise_vocab_size, self.needle_vocab_size, self.maximum_count) != (256, 10, 128):
            raise ValueError("v19 requires 256 noise tokens, 10 needle tokens, and count 1..128")
        if (self.n_layer, self.n_head, self.n_embd, self.n_inner) != (4, 4, 256, 1024):
            raise ValueError("v19 requires 4 layers, 4 heads, d_model=256, MLP=1024")
        if self.n_embd // self.n_head != 64:
            raise ValueError("v19 requires head_dim=64")
        ...
        result.update(
            {
                ...
                "direct_objective": "completion-only CE on COUNT,digits(n),NUM_END",
                "cot_objective": "completion-only CE on INDEX,digits(k),M_k,...,END,COUNT,digits(n),NUM_END",
                "number_tokenization": "all indices and final counts use shared decimal digit tokens D0..D9",
                "role_separation": "INDEX and COUNT role tokens disambiguate ordinal progress from the final scalar answer",
...
def _pair(distribution: str, alpha: float | None = None) -> list[RunSpec]:
    ...
    return [
        RunSpec(
            f"{distribution}_L1024_train128_eval128{suffix}_digit_{mode}",
            distribution,
            1024,
            128,
            128,
...
def canonical_run_specs() -> tuple[RunSpec, ...]:
    """Focused four-model digit-tokenization comparison for v19."""

    specs = _pair("uniform") + _pair("power", 1.5)
    for spec in specs:
        spec.validate()
    if len(specs) != 4 or len({spec.name for spec in specs}) != 4:
        raise AssertionError("canonical v19 suite must contain four unique runs")
    return tuple(specs)
...
def debug_run_specs() -> tuple[RunSpec, ...]:
    specs = tuple(
        RunSpec(f"debug_L32_train4_eval6_digit_{mode}", "uniform", 32, 4, 6, mode)
        for mode in ("direct", "cot")
    )
    for spec in specs:
        spec.validate()
