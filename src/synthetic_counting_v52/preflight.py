...
def run_preflight(run_dir: str | Path) -> dict[str, Any]:
    """Audit v52 support, permutation, endpoint, and component weights."""

    root = Path(run_dir)
    result = _base_preflight(root, expected_version="v52")
    cfg = config_from_dict(json.loads((root / "config.json").read_text(encoding="utf-8")))
    if not cfg.permute_task_context_tokens:
        raise ValueError("v52 requires permuted counting-task contexts")
    if cfg.train_steps != 10_000:
        raise ValueError("v52 requires the fixed 10,000-step endpoint")
    if (cfg.n_layer, cfg.n_head, cfg.n_embd, cfg.n_inner) != (4, 4, 256, 1024):
        raise ValueError("v52 requires the fixed 4L/4H/256D/1024-MLP architecture")
    if not cfg.task_output_trace_delimiters_as_structure:
        raise ValueError("v52 requires grammar-balanced separator partitioning")
    if (
        cfg.task_output_count_weight,
        cfg.task_output_trace_weight,
        cfg.task_output_structure_weight,
    ) != (8.0, 16.0, 16.0):
        raise ValueError("v52 requires count/trace/structure weights 8/16/16")
    ...
    if sorted(int(example.count or 0) for example in examples) != list(range(1, 11)):
        raise ValueError("v52 permutation audit is not balanced over counts 1..10")
    ...
    if changed_order != len(examples):
        raise ValueError("at least one v52 audit window was not actually permuted")
    ...
    result.update(
        {
            ...
            "task_output_component_weights": {
                "final_count": 8.0,
                "trace_marker_identity": 16.0,
                "grammar_structure": 16.0,
            },
            "serialized_trace_changed_from_v51": False,
            "training_change_from_v51": "marker-identity trace aggregate weight 8 -> 16 only",
    ...
    output = root / "analysis" / "preflight_v52.json"
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Audit v52 before optimization")
