...
`eval_examples_per_count` controls balanced fixed-suite size. Its main default is 50,
so counts 1–10 produce 500 examples per raw/task/mixture suite. A periodic checkpoint
...
enabled model, and evaluation examples per count. The reference defaults enable only
the RoPE nonthinking/thinking pair with `task_occurrence_ratio = 1.0`; RPE and mixed/raw
training are explicit ablations. At least one model switch must be enabled. New configs
default to the 1,500-step boundary. Legacy v16_2 configs missing the
...
## Full v10 mechanism-analysis port

The v10 mechanism suite is available as a separate, deterministic post-training pass.
It requires the final RoPE nonthinking and thinking checkpoints plus the saved fixed
evaluation suites. Head selection and intervention reporting use disjoint examples;
representation centroids are fit on the training region and evaluated or patched on
held-out receivers.

```bash
python scripts/run_v16_2_v10_port_analysis.py RUN_DIR --device cuda
python scripts/plot_v16_2_v10_port_analysis.py RUN_DIR
python scripts/build_v16_2_interactive_geometry.py RUN_DIR
python scripts/build_v16_2_v10_full_report.py --run-dir RUN_DIR
python scripts/validate_v16_2_v10_artifacts.py RUN_DIR
python scripts/audit_v16_2_report_figures.py RUN_DIR
```

The analysis writes atomic CSV tables and a SHA-256 manifest under
`analysis/v10_port/`. It covers two- and three-dimensional count/progress geometry,
global and query-local head ablation, count-preserving retrieval corruption and head
patching, continue/stop successor patching, residual logit lenses, MLP feature
concentration and patching, final-query count transport, length-preserving trace
conflicts, final-bridge component patching, early-stop residual patches, and
head-to-state/state-to-head bidirectional tests.

`interactive_hidden_state_pca.csv` adds the browser-side 3D view. For each of 21
checkpoints, four semantic sites, and the embedding output plus layers 1--4, it first
averages held-out hidden states within each count/progress label and then fits PCA to
the ten 256-dimensional centroids. The report can select any three of PC1--PC6, label
range, checkpoint, site, and depth. PCA is fit independently in every view; use the
displayed variance, effective dimension, adjacent-direction cosine, and path
straightness for cross-checkpoint comparisons rather than treating screen direction as
a shared coordinate system.

The single report is `v16_2_full_causal_report.html`. It embeds every figure, introduces
each metric before its first use, places an axis/baseline/aggregation guide immediately
before every figure, and includes a v10-to-v16.2 crosswalk with the evidence level of
each claim. Known legacy HTML aliases are removed so the run directory has one report
entry point.
The figure-audit command hashes every embedded PNG back to its source and writes contact
sheets plus `embedded_figure_audit.csv` under `analysis/v10_port/qa/` for overlap review.
