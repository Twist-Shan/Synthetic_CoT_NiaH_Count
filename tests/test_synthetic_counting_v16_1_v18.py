...
from synthetic_counting_v11.config import (
    preset_config as legacy_preset_config,
    same_experiment_except_version,
)
...
def test_v16_1_uses_split_local_indexed_windows_without_replacement():
    cfg = legacy_preset_config("v16_1", "debug", device="cpu")
...
def test_v16_1_sampler_has_no_repeat_before_epoch_end_and_resumes_exactly():
    cfg = legacy_preset_config("v16_1", "debug", device="cpu")
...
def test_v16_1_sampler_materializes_corpus_split_only_once(monkeypatch):
    cfg = legacy_preset_config("v16_1", "debug", device="cpu")
    vocab = Vocab.build(cfg)
...
def test_v16_1_sampling_is_natural_not_forced_uniform():
    cfg = legacy_preset_config("v16_1", "debug", device="cpu")
    vocab = Vocab.build(cfg)
...
def test_version_label_migration_requires_identical_experiment_settings():
    cfg = legacy_preset_config("v16_1", "debug", device="cpu")
    requested = cfg.to_dict()
    previous = cfg.to_dict()
    previous["version"] = "previous_label"
    previous["architecture_note"] = "metadata from the previous label"
    assert same_experiment_except_version(previous, requested)

    previous["batch_size"] = cfg.batch_size + 1
    assert not same_experiment_except_version(previous, requested)
...
def test_v16_1_v18_notebooks_mount_drive_first_and_compile():
    for version in ("v16_1", "v18"):
